...
void process_ui_event(void* event)
{
    ...
    switch (mduievt->event)
    {
        case UTM_CARTRIDGE_INSERTED:
            ...
            isCartridgeInserted = false;  //Temporarily false while we reset hardware

            reset_transfer_machines();  //Reset PIO TX/RX machines to a clean state
            abort_write_gap_alarm();    //Cancel any pending write gap alarm
            disable_DMAs(true);         //Disable any active DMA transfers
            disable_DMAs(false);
            gpio_put(MD_HEAD_DIR, 1);   //Set dir to input
...
void init_PIO_machines()
{
    ...
    */
    ...
    sm_config_set_clkdiv(&rx_cfg, (clock_get_hz(clk_sys) / 100000) / 20); //we run 20 times faster than the ULA
    ...
    */
    ...
    sm_config_set_clkdiv(&tx_cfg, (clock_get_hz(clk_sys) / 100000) / 16.0f); //we run 16 times faster than the ULA
    ...
    */
    ...
    sm_config_set_clkdiv(&shift_cfg, (clock_get_hz(clk_sys) / 21500) / 32); //we run 32 times faster than the serial clock (21.5Khz)
